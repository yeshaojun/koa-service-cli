...
const { getNpmSemverVersion } = require("../lib/getNpmInfo");
...
module.exports = async (name) => {
  ...
  const lastVersion = await getNpmSemverVersion(pg.version, pg.name);
  console.log("lastVersion", lastVersion);
  if (lastVersion && semver.gt(lastVersion, pg.version)) {
    console.log(
      chalk.gray(`
      欢迎使用${pg.name}脚手架
      更新提示：当前版本为${pg.version},最新版本为:${lastVersion}
